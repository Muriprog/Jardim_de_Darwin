...
class Jogo:
    ...
    def iniciar_jogo(self):
        ...
        self.master.after(2000, self.remover_texto_e_comecar_jogo)
    ...
    def mover_esquerda(self, event):
        if not self.imortal:
            self.canvas.move(self.passaro_id, -15, 0)  # Movendo o pássaro para a esquerda
            self.tentilhao_posicao = self.canvas.coords(self.passaro_id)[:2]
    ...
    def mostrar_segunda_fase(self):
        self.canvas.create_text(self.canvas.winfo_width()//2, self.canvas.winfo_height()//2, text="PROXIMA FASE", font=("Arial", 30), fill="white", tags="segunda_fase")

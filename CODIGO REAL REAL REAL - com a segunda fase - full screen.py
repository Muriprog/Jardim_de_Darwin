...
class QuizApp:
    ...
    def responder_pergunta(self):
        ...
        if resposta_usuario == resposta_correta:
            ...
            self.jogo.atualizar_pontuacao()  # Atualiza a pontuação na tela
        ...
        if self.current_question_index >= len(self.round_questions):
            self.master.destroy()
            self.jogo.reiniciar_jogo(manter_pontuacao=True, aumentar_velocidade=True)
...
class Jogo:
    def __init__(self, master):
        ...
        self.pontuacao = 0
        self.velocidade = 1  # Inicializa a velocidade como 1
        ...
        self.master.bind("<Configure>", self.redimensionar_canvas)

    def reiniciar_jogo(self, manter_pontuacao=False, aumentar_velocidade=False):
        self.canvas.delete("all")
        if not manter_pontuacao:
            self.pontuacao = 0
    ...
    def desenhar_animais(self):
        ...
        for _ in range(5):
            ...
            self.insetos.append({"id": inseto_id, "dx": random.choice([-1, 1]) * self.velocidade, "dy": random.choice([-1, 1]) * self.velocidade})

        for _ in range(5):
            x, y = random.randint(100, 700), random.randint(0, 550)
            besouro_id = self.canvas.create_image(x, y, anchor=tk.NW, image=self.besouro_img)
            self.besouros.append({"id": besouro_id, "dx": random.choice([-1, 1]) * self.velocidade, "dy": random.choice([-1, 1]) * self.velocidade})
